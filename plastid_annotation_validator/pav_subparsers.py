...
def _add_annotation_group(parser):
    """Add genome annotation argument group to parser"""
    optional_group_annotation = parser.add_argument_group('Genome annotation')

    optional_group_annotation.add_argument('--linearise_gene',
                                 type=str,
                                 nargs='+',
                                 default=['psbA'],
                                 metavar='GENE_NAME',
    ...
    optional_group_annotation.add_argument('--no-filter',
                                action='store_true',
                                default=False,
                                dest='no_filter',
                                help='Use the `--no-filter` flag in the chloe annotate command. Default is: %(default)s')
...
def _add_alignment_group(parser):
    ...
    optional_group_alignment.add_argument('--no_per_sample_alignments_with_refs',
                                        action='store_true',
                                        default=False,
                                        dest='no_per_sample_alignments_with_refs',
                                        help='Do not generate per-sample alignments with reference genes. '
                                             'Default is: %(default)s')
    optional_group_alignment.add_argument('--no_per_gene_alignments_with_refs',
                                        action='store_true',
                                        default=False,
                                        dest='no_per_gene_alignments_with_refs',
                                        help='Do not generate per-gene alignments with reference genes '
                                             '(combined samples). Default is: %(default)s')
    optional_group_alignment.add_argument('--no_sample_only_alignments',
                                        action='store_true',
                                        default=False,
                                        dest='no_sample_only_alignments',
                                        help='Do not generate sample-only alignments (no reference sequences). '
                                             'Default is: %(default)s')
...
def _add_general_options_group(parser):
...
def annotate_and_check_parser(subparsers):
    ...
    _add_annotation_group(parser_annotate_and_check)
    ...
    _add_general_options_group(parser_annotate_and_check)
...
def check_parser(subparsers):
    ...
    _add_general_options_group(parser_check)
